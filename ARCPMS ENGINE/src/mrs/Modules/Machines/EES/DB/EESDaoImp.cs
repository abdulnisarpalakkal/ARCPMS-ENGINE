...
namespace ARCPMS_ENGINE.src.mrs.Modules.Machines.EES.DB
{
    class EESDaoImp : EESDaoService
    {
        ...
        public bool UpdateMachineBlockStatusForPMS(string machine_code, bool blockStatus)
        {
            ...
            try
            {
                ...
                using (OracleConnection con = new DBConnection().getDBConnection())
                {
                    ...
                    OracleCommand command = con.CreateCommand();
                    string sql = "update L2_EES_MASTER set IS_IN_PROCESS ='" + (blockStatus ? 1 : 0) + "' where MACHINE_CODE = '" + machine_code + "'"
                                 + " and IS_IN_PROCESS!= " + (blockStatus ? 1 : 0);
                    command.CommandText = sql;
                    command.CommandType = CommandType.Text;
        ...
        public bool IsEESBlockedInDBForPMS(string machineName)
        {
            ...
            try
            {
                ...
                using (OracleConnection con = new DBConnection().getDBConnection())
                {
                    if (con.State == ConnectionState.Closed) con.Open();
                    OracleCommand command = con.CreateCommand();
                    string sql = "SELECT IS_IN_PROCESS FROM L2_EES_MASTER WHERE MACHINE_CODE ='" + machineName + "'";
                    command.CommandText = sql;
                    command.CommandType = CommandType.Text;
                    bResult = Convert.ToInt32(command.ExecuteScalar());
                    bOk = bResult == 1;
                ...
                using (OracleConnection con = new DBConnection().getDBConnection())
                {
                    ...
                    if (objEESData.isEntry)
                        sql = "UPDATE L2_CUSTOMERS SET ENTRY_NORTH_IMG = '" + imageNorth + "'," +
                             " ENTRY_SOUTH_IMG ='" + imageSouth + "' WHERE customer_id = '" + objEESData.customerPkId + "' and QUEUE_ID =" + objEESData.queueId;
                    else
        ...
        public bool UpdateGateEntryStatus(string machinCode, short status)
        {
            bool bOk = false;
            try
            {
                ...
                using (OracleConnection con = new DBConnection().getDBConnection())
                {
                    ...
                    OracleCommand command = con.CreateCommand();
                    string sql = "update L2_EES_MASTER set ENTRY_STATUS ='" + status + "' where MACHINE_CODE = '" + machinCode + "'"
                                 + " and ENTRY_STATUS!= " + status;
                    command.CommandText = sql;
                    command.CommandType = CommandType.Text;
                    bOk = command.ExecuteNonQuery() > 0;
