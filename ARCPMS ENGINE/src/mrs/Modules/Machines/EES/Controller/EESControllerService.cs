...
namespace ARCPMS_ENGINE.src.mrs.Modules.Machines.EES.Controller
{
    interface EESControllerService 
    {
        ...
        void CheckGateEntryStatus(EESData objEESData);
        GlobalValues.GATE_ENTRY_STATUS GetGateEntryStatus(EESData objEESData);
        bool IsEESReadyForCarEntry(EESData objEESData);
        bool GetVehicleDetectorStatus(Model.EESData objEESData);

        bool ShowTrigger(EESData objEESData);
    }
}
