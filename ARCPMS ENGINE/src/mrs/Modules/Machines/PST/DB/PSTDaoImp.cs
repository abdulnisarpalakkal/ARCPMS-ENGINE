...
namespace ARCPMS_ENGINE.src.mrs.Modules.Machines.PST.DB
{
    class PSTDaoImp:PSTDaoService
    {
        ...
        public int GetPST100PathId(bool isStore)
        {
            int pathID = 0;
            try
            {
                using (OracleConnection con = new DBConnection().getDBConnection())
                {
                    ...
                    using (OracleCommand command = con.CreateCommand())
                    {
                        ...
                        command.CommandText = "PVL_MANIPULATION.find_path_for_pst100_main";
                        command.Parameters.Add("is_get", OracleDbType.Int16, (isStore ? 0 : 1), ParameterDirection.Input);
        ...
        public void UpdateAfterPST100Task(int pathId)
        {
            try
            {
                using (OracleConnection con = new DBConnection().getDBConnection())
