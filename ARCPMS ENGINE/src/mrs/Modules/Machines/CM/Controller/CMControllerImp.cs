    ...
    class CMControllerImp : CommonServicesForMachines,CMControllerService
    {
                ...
                using (OpcOperationsService opcd = new OpcOperationsImp(OpcConnection.GetOPCServerConnection()))
                {
                    foreach (CMData objCMData in cmList)
                    {
                        ...
                        if (opcd.IsMachineHealthy(objCMData.cmChannel + "." + objCMData.machineCode + "." + OpcTags.CM_L2_Min_Window_Limit) == true)
                        {
                            ...
                            {
                                dataValueInBool = opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.LCM_L2_AUTO_READY);
                                UpdateCMBoolData(objCMData.machineCode, OpcTags.LCM_L2_AUTO_READY, dataValueInBool);
                            }
                            else
                            {
                                dataValueInBool = opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.UCM_L2_AUTO_READY);
                                UpdateCMBoolData(objCMData.machineCode, OpcTags.UCM_L2_AUTO_READY, dataValueInBool);
                            }
                            ...
                            //{
                            //    dataValueInBool = dataValueInBool && opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.LCM_L2_AUTO_READY);
                            //    if (dataValue > 0) UpdateCMBoolData(objCMData.machineCode, OpcTags.LCM_L2_AUTO_READY, dataValueInBool);
                            //}
                            //else
                            //{
                            //    dataValueInBool = dataValueInBool && opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.UCM_L2_AUTO_READY);
                            //    if (dataValue > 0) UpdateCMBoolData(objCMData.machineCode, OpcTags.UCM_L2_AUTO_READY, dataValueInBool);
        ...
        public bool CMRotate(Model.CMData objCMData)
        {
            ...
            do
            {
                ...
                try
                {
                    if (!objCMDaoService.GetCMRotationEnabledStatus(objCMData.machineCode, objCMData.requestType))
                        break;
                    isCMHealthy = CheckCMHealthy(objCMData) ;
                    ...
                    using (OpcOperationsService opcd = new OpcOperationsImp(OpcConnection.GetOPCServerConnection()))
                    {
                            ...
                            if (isPathClear)
                            {
                               ...
                               // else if (opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.LCM_TT_at_180_Degrees))
                                    opcd.WriteTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.LCM_L2_TT_ROT, true);
        ...
        public bool ConfirmRotateCmdAccepted(CMData objCMData)
        {
            ...
            do
            {
                ...
                using (OpcOperationsService opcd = new OpcOperationsImp(OpcConnection.GetOPCServerConnection()))
                {
                    isConfirmed = opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.LCM_L2_CM_IN_ROTATION);
        ...
        public bool ClearPath(Model.CMData objCMData)
        {
           ...
           // bool isCollapseMachineAvailable = true;
            ...
            try
            {
                // ErrorReportEngine rpt = new ErrorReportEngine();
                opcd = new OpcOperationsImp(OpcConnection.GetOPCServerConnection());
              
               
               
                //isExit = dbpm.isExit(queueId);
                ...
                //{
                    ...
                    objCMDaoService.UpdateIntValueUsingMachineCode(objCMData.machineCode, "CHANGED_HOME_AISLE", objCMData.positionAisle);
                    ...
                    if (objCMData.CMPresentOnSide)
                    {
                        ...
                        while (true);
                        objCMDaoService.UpdateIntValueUsingMachineCode(colCMData.machineCode, "CHANGED_HOME_AISLE", colCMData.positionAisle);
        ...
        public bool ClearForRotaion(Model.CMData objCMData)
        {
           ...
           // int checkCountLimit = 0;
            ...
            try
            {
                ...
                if (objCMData.carType != ARCPMS_ENGINE.src.mrs.Global.GlobalValues.CAR_TYPE.low)
                    leftMaxAisle = 8;
                else
                    leftMaxAisle = 1;
               ...
               // {
                  ...
                  //  if (checkCount >= checkCountLimit)
                    ...
                    if (!isRotate && objCMData.cmdVal!=0)
                    {
                        ...
                        bool caseHighCar1 = false;
                        bool caseHighCar2 = false;
                        ...
                        caseHighCar1 = objCMData.carType != ARCPMS_ENGINE.src.mrs.Global.GlobalValues.CAR_TYPE.low 
                            && objCMData.positionAisle <= 8 && objCMData.positionAisle != 0; //move high car cm to 9
                        //move all other CMs to 12 for rotation
                        caseHighCar2 = //objCMData.isHighCar && 
                           objCMData.positionAisle <= 10 && objCMData.positionAisle != 0 && !objCMData.machineCode.Equals("LCM_FLR04_01");
                        bool success = false;
                        if (caseHighCar1)
                        {
                            pushRefCMData.destAisle = 9;
                            pushRefCMData.destRow = 2;
                            success=CMMove(pushRefCMData);
                            if (success)
                                CheckCMCommandDone(pushRefCMData);
                        }
                        else if (caseHighCar2)
                        {
                            pushRefCMData.destAisle = 12;
                            pushRefCMData.destRow = 2;
                            success=CMMove(pushRefCMData);
                            if (success)
                                CheckCMCommandDone(pushRefCMData);
                        ...
                        if (!(leftAisleGap >= expectedLeftgap) && IsLeftCMHealthy && isLeftCMAvailable && !caseHighCar1 && !caseHighCar2 && !isLeftCMOff)
                        {
                            pushRefCMData.destAisle = objCMData.positionAisle - 2;
                            pushRefCMData.destRow = 2;
                        ...
                        if (!(rightAisleGap >= expectedRightgap) && IsRightCMHealthy && isRightCMAvailable && !caseHighCar1 && !caseHighCar2 && !isRightCMOff)
                        {
                            pushRefCMData.destAisle = objCMData.positionAisle + 2;
                            pushRefCMData.destRow = 2;
                        ...
                        //only for LCM5 bug No: 04.08.15.8
                        if ( objCMData.machineCode.Equals("LCM_FLR4_05") && objCMData.positionAisle == 37 && objCMData.cmdVal==2)
                        {
                            pushRefCMData.destAisle = 36;
                            pushRefCMData.destRow = 2;
                            success = CMMove(pushRefCMData);
                            if (success)
                                CheckCMCommandDone(pushRefCMData);
            ...
            try
            {
                ...
                do
                {
                    ...
                    if (triggerAlreadyEnabled || objTriggerData.TriggerEnabled)
                    {
                        ...
                        if (objQueueControllerService.NeedToOptimizePath(objCMData.queueId))
                        {
                            break;
                        }
        ...
        public bool CheckCMRotateDone(Model.CMData objCMData)
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    result = opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.LCM_L2_ROTATE_DONE);
        ...
        public string getREMCode(string machineCode)
        {
            ...
            if (remCode.Contains("FLR0"))
                remCode = remCode.Replace("FLR0", "FLR");
            return remCode;
        }
        public bool CheckCMHealthy(Model.CMData objCMData)
        {
            ...
            using (OpcOperationsService opcd = new OpcOperationsImp(OpcConnection.GetOPCServerConnection()))
            {
                ...
                {
                    if (objCMData.machineCode.Contains("LCM"))
                        isHealthy = opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.LCM_L2_AUTO_READY);
                    else
                        isHealthy = opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.UCM_L2_AUTO_READY);
        ...
        public TriggerData NeedToShowTrigger(Model.CMData objCMData)
        {
            ...
            using (OpcOperationsService opcd = new OpcOperationsImp(OpcConnection.GetOPCServerConnection()))
            {
                do
                {
                        ...
                        if (objCMData.machineCode.Contains("LCM"))
                            needToShow = !opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.LCM_L2_AUTO_READY);
                        else
                            needToShow = !opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.UCM_L2_AUTO_READY);
                        ...
                        if (objCMData.machineCode.Contains("LCM") && objCMData.command != OpcTags.LCM_L2_TT_ROT)
                        {
        ...
        public bool CheckCMIsRotating(CMData objCMData)
        {
            bool isCMRotating = false;
             using (OpcOperationsService opcd = new OpcOperationsImp(OpcConnection.GetOPCServerConnection()))
             {
                 //isCMRotating=!opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.LCM_L2_ROTATE_DONE);
                 isCMRotating = opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.LCM_L2_CM_IN_ROTATION);
             }
        ...
        public bool ConfirmCMRotationStatus(CMData objCMData)
        {
            ...
            if (!isCMRotating)
            {
                ...
                if (!string.IsNullOrEmpty(objTriggerData.command))
                    isCMRotating = objTriggerData.command.Equals(OpcTags.LCM_L2_TT_ROT);
            }
            ...
            try
            {
                using (OpcOperationsService opcd = new OpcOperationsImp(OpcConnection.GetOPCServerConnection()))
                {
                    if (opcd.IsMachineHealthy(objCMData.cmChannel + "." + objCMData.machineCode + "." + OpcTags.CM_L2_Destination_Aisle))
                    {
                        ...
                        isIdeal = isIdeal && !opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.CM_L2_CM_IN_BUSY);
        ...
        public bool IsDestInSamePos(CMData objCMData)
        {
            bool isSameSide = false;
            using (OpcOperationsService opcd = new OpcOperationsImp(OpcConnection.GetOPCServerConnection()))
            {
                //objCMData.positionAisle = opcd.ReadTag<Int32>(objCMData.cmChannel, objCMData.machineCode, OpcTags.CM_Position_for_L2);
                //cmAtNorth = opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.CM_AT_NORTH);
                isSameSide=objCMData.positionAisle == objCMData.destAisle;
                isSameSide = isSameSide && IsRowInNorthSide(objCMData.destRow) == opcd.ReadTag<bool>(objCMData.cmChannel, objCMData.machineCode, OpcTags.CM_AT_NORTH);
            }
            return isSameSide;
