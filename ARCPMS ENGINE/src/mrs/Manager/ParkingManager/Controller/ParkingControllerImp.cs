...
using ARCPMS_ENGINE.src.mrs.Manager.CarWashManager.Controller;
...
namespace ARCPMS_ENGINE.src.mrs.Manager.ParkingManager.Controller
{
    class ParkingControllerImp: ParkingControllerService
    {
        ...
        CarWashControllerService objCarWashControllerService = null;
        ...
        public void EntryCarProcessing(QueueData objQueueData)
        {
            ...
            try
            {
                ...
                if (pathStartFlag == 0)
                {
                    ...
                    #region car type checking 
                    GlobalValues.CAR_TYPE carType = objEESControllerService.GetCarType(objEESData);
                    bool needUpdateCarType = false;
                    needUpdateCarType = objQueueData.carType == GlobalValues.CAR_TYPE.low
                                         && (carType == GlobalValues.CAR_TYPE.high
                                         || carType == GlobalValues.CAR_TYPE.medium);

                    needUpdateCarType = needUpdateCarType || objQueueData.carType == GlobalValues.CAR_TYPE.medium
                                         && carType == GlobalValues.CAR_TYPE.high;
                    ...
                    do
                    {
                        ...
                        if (getCarReady)
                        {
                            objEESData.command = OpcTags.EES_Get_Car;
        ...
        public void ExcecuteCommands(QueueData objQueueData)
        {
            ...
            if (objQueueControllerService == null) objQueueControllerService = new QueueControllerImp();
            if (objCarWashControllerService == null) objCarWashControllerService = new CarWashControllerImp();
                    ...
                    switch (pathDetails.command)
                    {
                        ...
                        case OpcTags.CM_L2_Put_Cmd:
                            ...
                            if (pathDetails.interactMachineCode.Contains("PVL"))
                            {
                                ...
                                do
                                {
                                    Logger.WriteLogger(GlobalValues.PMS_LOG, "ExcecuteCommandsForPMS:>Entering PVL Ready check :  Path Id=" + pathDetails.queueId
                        + " ; Is ready " + objPVLData.machineCode + "?");
                                    isPVLReady = objPVLControllerService.ConfirmPVLReadyPUTByCM(objPVLData.machineCode);
                                    Logger.WriteLogger(GlobalValues.PMS_LOG, "ExcecuteCommandsForPMS:>Exit PVL Ready Check:  Path Id=" + pathDetails.queueId
                       + " ; Is ready " + objPVLData.machineCode + "= " + isPVLReady);
                            ...
                            else if (pathDetails.interactMachineCode.Contains("EES"))
                            {
                                ...
                                do
                                {
                                    ...
                                    if (!isEESReady)
                                    {
                                        ...
                                        {
                                            autoRefreshCnt++;
                                            if (autoRefreshCnt > (pmsMode == 1 ? (autoRefreshLmt + 150) : autoRefreshLmt) )
                                            {
                                                Logger.WriteLogger(GlobalValues.PARKING_LOG, "Queue Id:" + objCMData.queueId
                        ...
                        case OpcTags.LCM_L2_TT_ROT:
                            ...
                            bool isSkipped = !objCMDaoService.GetCMRotationEnabledStatus(objCMData.machineCode, objCMData.requestType);
                            ...
                            if (isAlreadyRotated || isSkipped)
                                    objErrorControllerService.UpdateLiveCommandOfMachine(objErrorData); //for updating the sequence number which will trigger path details and  updat status 2
                        ...
                        case OpcTags.EES_Payment_Is_Done:
                            ...
                            break;
                        #endregion
                        #region WASH_CarWash_Start_Cycle
                        case OpcTags.WASH_CarWash_Start_Cycle:
                            ...
                            objErrorData = ConvertToErrorData(pathDetails);
                            objErrorControllerService.UpdateLiveCommandOfMachine(objErrorData);
                            if (objQueueData.carType == ARCPMS_ENGINE.src.mrs.Global.GlobalValues.CAR_TYPE.high)

                                objCarWashControllerService.SetHighCarBitForCarWash(objVLCData, true);
                            else
                                objCarWashControllerService.SetHighCarBitForCarWash(objVLCData, false);

                            objVLCData.isDone = objCarWashControllerService.StartCarWash(objVLCData);
                           
                            objErrorControllerService.UpdateLiveCommandStatusOfMachine(objVLCData.machineCode, true);
                            if (pathDetails.dynamicCall)
                            {
