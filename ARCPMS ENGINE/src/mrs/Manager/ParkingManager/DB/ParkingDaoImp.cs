...
namespace ARCPMS_ENGINE.src.mrs.Manager.ParkingManager.DB
{
    class ParkingDaoImp:ParkingDaoService
    {
        ...
        public bool IsRotated(int queueId)
        {
            ...
            try
            {
                using (OracleConnection con = new DBConnection().getDBConnection())
                {
                    if (con.State == ConnectionState.Closed) con.Open();
                    OracleCommand command = con.CreateCommand();
                    string sql = "select ct.is_rotate from L2_CUSTOMERS ct inner join l2_ees_queue q on ct.CUSTOMER_ID=q.customer_pk_id where  q.id=" + queueId;
                    command.CommandText = sql;
        ...
        public bool UpdateRotaion(int queueId,bool rotateStatus)
        {
            ...
            try
            {
                using (OracleConnection con = new DBConnection().getDBConnection())
                {
                    ...
                    OracleCommand command = con.CreateCommand();
                    string sql = "update L2_CUSTOMERS ct set ct.is_rotate=" + (rotateStatus?1:0) + " where ct.CUSTOMER_ID=(select q.customer_pk_id from l2_ees_queue q where q.id=" + queueId + ")";
                    command.CommandText = sql;
                    command.CommandType = CommandType.Text;
                    command.ExecuteNonQuery();
        ...
        public string GetCardIdUsingQueueId(int  queueId) // In EFT, value is refering directly to card id in customer table
        {
            string cardId = null;
            try
            {
