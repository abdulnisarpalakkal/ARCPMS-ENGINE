...
namespace ARCPMS_ENGINE.src.mrs.Manager.PalletManager.Controller
{
    class PalletManagerImp:PalletManagerService
    {
        ...
        //List<EESData> RequestQueueList = new List<EESData>();
            ...
            while (!_shouldStop)
            {
                try
                {
                    ...
                    foreach (EESData objEESData in eesList)
                    {
                        try
                        {
                            //updating gate entry status for ramp display
                            objEESControllerService.CheckGateEntryStatus(objEESData);
                            //check status of ees.
                            eesLocked = false;
                ...
                try
                {
                    ...
                    foreach (EESData objEESData in scanningEESList)
                    {
                        ...
                        if (needToFeedPallet)
                            ...
                            Task.Factory.StartNew(() => GetEESByPS(new PSData(), objRefEESData));
                    }
                }
                catch (Exception ex)
                {
                        ...
                        if (PMS_MODE == 3)//Normal Mode
                        {
                            ...
                            if (isPSTFull)
                            {
                                ...
                                else if ((exitEESWithPallet > 0 && hasPalletOnPS) || (exitEESWithPallet > 1 && !hasPalletOnPS))
                                {
                                    ...
                                    {
                                        Task.Factory.StartNew(() => GetPSTByLCM(objRefPSTData));
                                    }
                            ...
                            else if (isPSTEmpty)
                            {
                                ...
                                else if ((entryEESWithoutPallet > 0 && !hasPalletOnPS) || (entryEESWithoutPallet > 1 && hasPalletOnPS))
                                {
                                    ...
                                    {
                                        Task.Factory.StartNew(() => PutPSTByLCM(objRefPSTData));
                                    }
        ...
        public bool PutEESByPS(PSData objPSData, EESData objEESData)
        {
            ...
            try
            {
                ...
                if (IsPriorRequestAvailable(objEESData) && objEESData.priorityTryCount<=1)
                {
                    IncrementPriorityTryCount(objEESData);
                    return false;
                }
                ResetPriority(objEESData);
                ...
                if (psBlocked && eesBlocked)
                {
                    ...
                    success = objPSControllerService.PSPutToEES(objPSData, objEESData);
                    success = success && objPSControllerService.CheckPSCommandDone(objPSData, null, objEESData);
        ...
        public bool GetEESByPS(PSData objPSData, EESData objEESData)
        {
            ...
            try
            {
                ...
                if (psBlocked && eesBlocked)
                {
                    ...
                    objErrorData = ConvertPSDataToErrorData(objPSData);
                    objErrorControllerService.UpdateLiveCommandOfMachine(objErrorData);
                    success = objPSControllerService.PSGetFromEES(objPSData, objEESData);
                    success = success && objPSControllerService.CheckPSCommandDone(objPSData, null, objEESData);
        ...
        public bool GetPVLByUCM(PVLData objPVLData)
        {
            ...
            if (objPVLManagerService == null) objPVLManagerService = new PVLManagerImp();
        ...
        public bool GetPSTByLCM(PSTData objPSTData)
        {
           // Logger.WriteLogger(GlobalValues.PMS_LOG, "Entered GetPSTByLCM : PVL=" + objPVLData.machineCode);
            if (objPSTControllerService == null) objPSTControllerService = new PSTControllerImp();
            ...
            try
            {
                ...
                if (pstBlocked)
                {
                    success = objPSTControllerService.ProcessPST100Request(true);
            ...
            {
                Logger.WriteLogger(GlobalValues.PMS_LOG, "Exception GetPVLByUCM : PVL=" + objPSTData.machineCode + " , error= " + ex.Message);
            }
            finally
            {
                if (!success && pstBlocked)
                    objPSTControllerService.UpdateMachineBlockStatus(objPSTData.machineCode, false);
                //Logger.WriteLogger(GlobalValues.PMS_LOG, "Exitting GetPVLByUCM : PVL=" + objPSTData.machineCode + " , success= " + success + ", pvlBlocked " + pvlBlocked);
            }

            return success;
        }

        public bool PutPSTByLCM(PSTData objPSTData)
        {
            // Logger.WriteLogger(GlobalValues.PMS_LOG, "Entered GetPSTByLCM : PVL=" + objPVLData.machineCode);
            if (objPSTControllerService == null) objPSTControllerService = new PSTControllerImp();
            ...
            try
            {
                ...
                if (pstBlocked)
                {
                    success = objPSTControllerService.ProcessPST100Request(false);
        ...
        public bool ClearPSFromUnderPST(PSData objPSData)
        {
            ...
            try
            {
                ...
                if (psBlocked )
                {
                    ...
                    success = success && objPSControllerService.CheckPSCommandDone(objPSData,null,null);
                }
        ...
        //newly added functions: 16Jan19
        public bool IsPriorRequestAvailable(EESData objEESData)
        {
            EESData priorEES = eesList.Find(e =>e.machineCode!=objEESData.machineCode 
                && e.psCode==objEESData.psCode && e.requestTime!=null && e.requestTime < objEESData.requestTime);

            return priorEES != null;
        }
        public void IncrementPriorityTryCount(EESData objEESData)
        {
            EESData ees = eesList.Find(e => e.machineCode.Equals(objEESData.machineCode));
            ees.priorityTryCount++;

        }
        public void InitiatePriority(EESData objEESData)
        {
            if (objPSControllerService == null)
                objPSControllerService = new PSControllerImp();

            ResetPriority(objEESData);
            PSData objPSData = objPSControllerService.GetPSDetailsIncludeAisle(objEESData.aisle);
            objEESData.requestTime = DateTime.Now;
            objEESData.psCode = objPSData.machineCode;



        }
        public void ResetPriority(EESData objEESData)
        {
            EESData ees = eesList.Find(e => e.machineCode.Equals(objEESData.machineCode));
            ees.priorityTryCount = 0;
            ees.psCode = null;
